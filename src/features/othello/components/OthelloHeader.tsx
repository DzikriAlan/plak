...
interface Props {
  onLoadOthelloExit: () => void
  isSoundOn: boolean
  isInviteVisible?: boolean
  isInviteLoading?: boolean
  onSubmitOthelloInvite?: () => void
  onLoadOthelloGuide: () => void
  onEditOthelloSound: () => void
}

export default function OthelloHeader({
  onLoadOthelloExit,
  isSoundOn,
  isInviteVisible = false,
  isInviteLoading = false,
  onSubmitOthelloInvite,
  onLoadOthelloGuide,
  onEditOthelloSound,
}: Props) {
  return (
    <header className="shrink-0 space-y-3">
      <div className="flex min-h-[48px] items-stretch gap-2">
        ...
        <GameSoundToggle isSoundOn={isSoundOn} className="w-[38px] shrink-0" onEditGameSound={onEditOthelloSound} />
      </div>
    </header>
  )
}
