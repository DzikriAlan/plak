...
interface Props {
  ...
  botScore: number
  moveTotal: number
  onClearDotsAndBoxesGame: () => void
}

export default function DotsAndBoxesResult({
  resultLabel,
  rivalLabel = 'Kotak bot',
  playerScore,
  botScore,
  moveTotal,
  onClearDotsAndBoxesGame,
}: Props) {
  const stats = [
    { id: 'player', label: 'Kotak kamu', value: String(playerScore) },
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/80 px-6 backdrop-blur-sm">
      <div className="w-full max-w-[360px] rounded-2xl border border-[#26262b] bg-[#121214] p-6 text-center">
        ...
        <button
          type="button"
          onClick={onClearDotsAndBoxesGame}
