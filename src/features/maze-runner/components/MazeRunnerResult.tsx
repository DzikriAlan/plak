...
interface Props {
  ...
  timeLabel: string
  moveTotal: number
  onSubmitMazeRunnerNextLevel: () => void
  onClearMazeRunnerLevel: () => void
}

export default function MazeRunnerResult({
  isCleared,
  levelLabel,
  timeLabel,
  moveTotal,
  onSubmitMazeRunnerNextLevel,
  onClearMazeRunnerLevel,
}: Props) {
  const stats = [
    { id: 'move', label: 'Moves', value: String(moveTotal) },
    { id: 'time', label: 'Time left', value: timeLabel },
  ]
    ...
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/80 px-6 backdrop-blur-sm">
      <div className="w-full max-w-[360px] rounded-2xl border border-[#26262b] bg-[#121214] p-6 text-center">
        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#a29d93]">
          {isCleared ? 'Sampai di gua' : 'Waktu habis'}
        </p>
        <p className="mt-2 text-3xl font-black uppercase leading-none text-[#f2ede1]">Level {levelLabel}</p>
